...
@ray.remote
def exec_func(
    plugin_func, arg_index, karg_keys, args, kargs, func, node_name, read_object
):
    if read_object and ray.get(
        ray.remote(**read_object.remote_args)(read_object.file_exists).remote(node_name)
    ):
        return ray.get(
            ray.remote(**read_object.remote_args)(read_object.read_task).remote(
                node_name
            )
        )
...
class RayWorkflow(BaseWorkflow):
    ...
    def set_storage(self, object_storage):
        ...
        (
            ...
            self.read_object,
            self.save_func,
        ) = object_storage.get_writer_funcs(self.workflow_name)
        ...
        for name, exec_node in self.graph_traversal():
            ...
            if func_node:
                ...
                actor = ray.remote(Actor).remote(
                    exec_node.class_spec, arg_index, karg_keys, args, kargs
                )
                ...
                result = actor.call_method.remote(
                    exec_node.method_name, arg_index, karg_keys, args, kargs
                )
