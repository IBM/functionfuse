...
class StorageWithSerializers:
    ...
    def pickle(self, obj, protocols):
        return safepickle(obj, self.persistent_serializers, protocols, self.reduced_serializers)
...
class PersistentUnpickler(pickle.Unpickler):
    ...
    def persistent_load(self, pid):
        ...
        if serializer:
            ...
            raise pickle.UnpicklingError(f"Deserialization error, unsupported persistent object {tag}.")
...
def safepickle(obj, persistent_serializers, protocols, reduced_serializers):
    ...
    if persistent_serializers or reduced_serializers:
        stream = io.BytesIO()
        p = PersistentPickler(stream, persistent_serializers, protocols) \
            if persistent_serializers else pickle.Pickler(stream)
